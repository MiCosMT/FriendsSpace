...
class UserController {
  async login(req, res) {
    try {
      ...
      if (usuarioBuscado.banned)
        return res
          .status(401)
          .json({ ok: false, mensaje: "Cuenta suspendida" });

      const token = generarToken(usuarioBuscado);
      res.cookie("access_token", token, cookieOpts);
      const { password: _, ...usuarioLimpio } = usuarioBuscado.toJSON();
    ...
    try {
      if (req.user.role !== "DEVELOPER")
        return res.status(403).json({
          ok: false,
          mensaje: "Solo los developers pueden crear admins",
        });
  ...
  async getAllUsers(req, res) {
    try {
      ...
      const interests = req.query.interests
        ? req.query.interests.split(",").map(Number).filter(Boolean)
        : [];
  ...
  async updateUser(req, res) {
    try {
      ...
      if (req.body.name) {
        const usuarioActual = await userService.getUserById(req.params.id);

        if (
          req.body.name.trim().toLowerCase() !==
          usuarioActual.name.toLowerCase()
        ) {
  ...
  async changePassword(req, res) {
    try {
      ...
      if (newPassword !== confirmPassword)
        return res
          .status(400)
          .json({ ok: false, mensaje: "Las contraseñas no coinciden" });

      const passError = userValidations.passwordValidation(newPassword);
      if (passError)
        return res.status(400).json({ ok: false, mensaje: passError });

      const nuevaVersion = await userService.changePassword(
        req.params.id,
        currentPassword,
        newPassword,
      );
      ...
      return res
        .status(500)
        .json({ ok: false, mensaje: "Error al cambiar la contraseña" });
    }
  }
  ...
  async updateAvatar(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ ok: true, url: nuevaUrl, usuario: usuarioActualizado });
    } catch (err) {
