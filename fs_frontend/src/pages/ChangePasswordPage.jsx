...
export default function ChangePasswordPage() {
  ...
  return (
    <Box sx={{ maxWidth: 520, mx: "auto", width: "100%", px: { xs: 2, sm: 3, md: 4 }, py: { xs: 2, md: 4 } }}>

      <Box sx={{ borderRadius: "20px", overflow: "hidden", background: theme.secondaryBack, border: `1px solid ${accent}20`, mb: 3 }}>
        <Box sx={{ height: { xs: 60, md: 80 }, background: `linear-gradient(135deg, ${accent}25, ${accent}05)` }} />
        <Box sx={{ px: { xs: 2, md: 3 }, pb: 3, mt: "-20px" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 1 }}>
            <Box sx={{ width: 44, height: 44, borderRadius: "12px", background: `${accent}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <LockIcon sx={{ color: accent, fontSize: 22 }} />
            </Box>
            <Box>
              <Typography sx={{ fontWeight: 800, fontSize: { xs: "1.1rem", md: "1.3rem" }, color: theme.primaryText, lineHeight: 1.2 }}>
                Cambiar contraseña
              </Typography>
              <Typography sx={{ fontSize: "0.75rem", color: theme.mutedText }}>
      ...
      <Box sx={sectionSx}>
        <Typography sx={{ fontWeight: 700, fontSize: "0.7rem", color: accent, letterSpacing: "0.08em", textTransform: "uppercase", mb: 2 }}>
          Contraseña actual
        </Typography>
        <TextField
          ...
          slotProps={{ input: { endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPasswords((p) => ({ ...p, current: !p.current }))} edge="end" sx={{ color: theme.mutedText }}><Box sx={{ display: "flex" }}>{showPasswords.current ? <VisibilityOff /> : <Visibility />}</Box></IconButton></InputAdornment> } }}
          sx={inputSx(false)}
        />
      </Box>
      ...
      <Box sx={sectionSx}>
        ...
        <TextField
          ...
          slotProps={{ input: { endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPasswords((p) => ({ ...p, new: !p.new }))} edge="end" sx={{ color: theme.mutedText }}><Box sx={{ display: "flex" }}>{showPasswords.new ? <VisibilityOff /> : <Visibility />}</Box></IconButton></InputAdornment> } }}
        />
        <TextField
          ...
          slotProps={{ input: { endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPasswords((p) => ({ ...p, confirm: !p.confirm }))} edge="end" sx={{ color: theme.mutedText }}><Box sx={{ display: "flex" }}>{showPasswords.confirm ? <VisibilityOff /> : <Visibility />}</Box></IconButton></InputAdornment> } }}
          sx={inputSx(form.confirmPassword.length > 0 && form.newPassword !== form.confirmPassword)}
        />
      </Box>

      {error && (
        <Typography sx={{ color: "#f44336", fontSize: "0.85rem", textAlign: "center", mb: 2, px: 2 }}>
          {error}
        </Typography>
      )}
      ...
      <Box sx={{ 
        ...
        <Button
          ...
          sx={{ borderColor: `${accent}50`, color: accent, borderRadius: "10px", textTransform: "none", fontWeight: 600, px: 3, py: { xs: 1.2, sm: 1 }, "&:hover": { borderColor: accent, background: `${accent}10` } }}
        >
          Volver
        </Button>
        <Button
          variant="contained" onClick={handleSubmit} disabled={isLoading}
          sx={{ background: `linear-gradient(135deg, ${accent}, ${theme.variantBack || accent})`, color: isDark ? "#1a1200" : "#fff", borderRadius: "10px", textTransform: "none", fontWeight: 700, px: 4, py: { xs: 1.2, sm: 1 }, flex: 1, "&:hover": { opacity: 0.9 }, "&.Mui-disabled": { background: theme.tertiaryBack, color: theme.mutedText } }}
        >
